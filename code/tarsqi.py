...
USAGE
   ...
   INPUT_TYPE.
      ...
      A string that determines the type of the data source, it
      determines what document processor is used and sets a default
      processing chain.

   PROCESSING_FLAGS.

      An optional set of parameters. Can be used to overrule default
      settings in the program as well as default settings given by the
      INPUT_TYPE. Currently defined flags are: trap_errors, log_level,
      content_tag, and extension. See the manual in docs/manual/ for
      more details on the parameters.

   INPUT/OUTPUT.

      Input and output files or directories. If the input is a
      directory than the output directory needs to exist.
...
class TarsqiControl:
    ...
    def __init__(self, id, opts, input, output):
        ...
        # crashed on a previous file it may end up being in a
        # different directory.
        os.chdir(TTK_ROOT)
    ...
    def apply_component(self, name, wrapper, infile, outfile):
        ...
        Component-level errors are trapped here if trap_errors is
        True. Those errors are now trapped here instead of in the
        component since we do not tell the component what the output
        file is.
        ...
        # The wrappers use the xml document and the content tag and
        # then (i) create fragments from the xml doc, (ii) process the
        # fragments, (iii) reinsert the fragments in the xml doc. The
        # code below then writes the xml doc to a file. Most wrappers
        # write the fragments to disk and call the components code on
        # that file, creating a processed fragment again as a disk
        # file. This should be streamlined, too much IO (but alwyas
        # keep the option of printing the file for debugging.
...
def _transform_values(dict):
    ...
    replaces some of them with other objects. Replaces 'True' with
    True, 'False' with False, and strings indicating an integer with
    that integer."""
...
def run_profiler(args):
    """Wrap running Tarsqi in the profiler. The problem is that this
     currently fails for unknown reasons. It runs through all the
