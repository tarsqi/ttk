...
# add javascript so we can click on an identifier and the see the sentence

# many evita-vg cases give a negative result, which is peculiar because the
# events were originally generated by Evita.
...
if __name__ == '__main__':
    ...
    for opt, val in opts:
        ...
        if opt == '--evita': run_evita()
        if opt == '--gutime': run_gutime()
        if opt == '--report': generate_report()
        if opt == '--purge': purge_result(args)
