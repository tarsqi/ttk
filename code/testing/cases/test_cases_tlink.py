...
Set of test cases for Tlinks. These tests are not intended to be a thorough
coverage test, but they try to hit enough cases so that errors are likely to be
found.

Cases are split into several categories, similar to how this was done for Slinks
and Alinks.
...
3- rule used (None if not tested)
4- character offsets of first event or time
5- character offsets of second event or time
6- a sentence

There is an optional seventh field which can be used to determine whether there
should or should not be a TLINK spanning the two events. Setting it to False
tests for the absence of an slink.

The end of the file contains specifications of test sets created from the
available categories. For example:

   S2T_TESTS = SIMPLE_S2T

The calling script (run_tests.py refers to these).
...
SIMPLE_S2T = [
    ...
    ('BEFORE', None, 'RULE-1', (5,8), (20,25),
     "John saw the miners build a fence."),

    ('AFTER', None, 'RULE-3', (3,7), (17,22),
     "He said the dogs slept."),

    ('BEFORE', None, 'RULE-10', (3,9), (13,17),
     "He wanted to walk."),
]
...
SIMPLE_BLINKER = [
    ...
    ('AFTER', None, None, (22,27), (35,39),
     "John saw an explosion today and in 2000."),

    ('IS_INCLUDED', None, None, (5,8), (22,27),
     "John saw an explosion today and in 2000."),

    ('IS_INCLUDED', None, 'Type 2 (on)', (4,9), (13,22), "Sue slept on Wednesday."),

    ('AFTER', None, 'RULE-5-29', (3,7), (17,22), "He said the dogs slept."),

    ('BEFORE', None, 'TimexLinking', (15,19), (24,28),
     "I was there in 2000 and 2015, and today too."),

    # this one is actually wrong, change when Blinker is fixed
    ('AFTER', None, 'TimexLinking', (15,19,), (34,39),
     "I was there in 2000 and 2015, and today too."),

    # this one is actually wrong, change when Blinker is fixed
    ('AFTER', None, 'TimexLinking', (24,28), (34,39),
     "I was there in 2000 and 2015, and today too."),
]
...
EVIDENTIAL = [
    ...
    ('EVIDENTIAL', 'AP900815-0044.xml', 'thatClause_NOT_that', (22,26), (72,79),
     "Soviet officials also said Soviet women, children and invalids would be allowed to leave Iraq."),
