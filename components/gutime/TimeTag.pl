...
#if set to 1, prints out non-consuming TIMEXes at the end of the
#printout, these timexes are for the begin and end points
my $printNCTs = 0;	
    ...
    if(/<(DATE_TIME|DATE|DL|DD)>\s*(.*)\s*<\/(DATE_TIME|DATE|DL|DD)>/oi)
    {
        ...
        if ($curTagType eq "DOCNO"){
            if ($temp =~ /[a-zA-Z]+(\d\d\d\d\d\d)\D/){
                #assume it's the 1900s, perhaps should change
                $temp = $1 + 19000000;				
