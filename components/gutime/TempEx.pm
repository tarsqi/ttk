...
#	} elsif ($phrase =~ /^the\s(1|last|final)\s(\d+)\s$TE_Units(s)?\sof$/){
	...
	#i.e. "a decade", "a couple years"
        ...
        # return unknown duration if no pattern applied
		print STDERR 'Unrecognized pattern in expressionToDuration. ';
		print STDERR "Processed (POS tags removed and number words converted) pattern is: $phrase\n";
        return "PXX";
	}
...
#	} elsif ($phrase =~ /^the\s(1|last|final)\s(\d+)\s$TE_Units(s)?\sof$/){
	...
	#i.e. "four minutes","a decade"
        ...
        # return variables for begin and end if no pattern applied
